...
def extract_metrics(results, decoder="linear"):
    data_ie = defaultdict(list)
    data_ie_status = defaultdict(list)
    ...
    for seed_data in seed_results.values():
        ...
        for name, val in res.items():
            delta_ie = val.get("delta_IE")

            if delta_ie is None or np.isnan(delta_ie):
                continue

            data_ie[name].append(delta_ie)
            data_ie_status[name].append(val.get("ie_status", ""))


    return data_ie, data_ie_status
...
def draw_points_generic(
    ...
    for surrogate in surrogates:
        ...
        x_vals = np.asarray(data_ie[surrogate])
        y_vals = 0.02 * (np.random.rand(len(x_vals)) - 0.5)

        statuses = data_ie_status[surrogate]

        for x, y, ie_stat in zip(x_vals, y_vals, statuses):

            marker = "o"

            if "holds" in ie_stat.lower():
                facecolor = "none"
                edgecolor = color
                linewidth = 1.2
            else:
...
def plot_panel_A(
    ...
    all_x = np.concatenate([data_ie[s] for s in surrogates])
...
def plot_group_panel(
    ...
    valid_labels = [
        l for l in target_labels if l in data_ie and len(data_ie[l]) > 0
    ]
    ...
    if valid_labels:
        ...
        x_vals = np.concatenate([data_ie[l] for l in valid_labels])
...
def make_figure(
    ...
    all_surrogates = list(data_ie.keys())
    ...
    status_handles = [
        mlines.Line2D([], [], color="black", marker="o", ls="", mfc="none",
                      markersize=13, label="IE holds"),
        mlines.Line2D([], [], color="black", marker="o", ls="", mfc="black",
                      markersize=13, label="IE violations"),
    ]
...
def main():
    ...
    if output_path is None:
        output_path = Path(
            f"Figure_IE_{task_tag}_{args.decoder}.pdf"
        )
